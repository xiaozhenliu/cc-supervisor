# 自主决策规则（Auto Mode Decision Rules）

本文档定义 `CC_MODE=auto` 模式下 OpenClaw Agent 的自主决策规则。目标是让 agent 能够完全自主地推进任务，只在真正无法继续的情况下才升级。
...
```
[cc-supervisor][auto] Escalation required:
...
Agent 在 auto 模式下收到 Stop 通知时，应按以下流程处理：
