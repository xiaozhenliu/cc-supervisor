...
# Write hook environment to file for reliable hook access
# (hook execution environment may not inherit shell variables reliably)
HOOK_ENV_FILE="${CC_PROJECT_DIR}/logs/hook.env"
mkdir -p "$(dirname "$HOOK_ENV_FILE")"
cat > "$HOOK_ENV_FILE" << EOF
# Hook environment variables (auto-generated by supervisor_run.sh)
# Source this file in hooks to ensure reliable access to OpenClaw routing info
...
EOF
log_info "Hook environment written to: $HOOK_ENV_FILE"
