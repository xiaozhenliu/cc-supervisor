...
# Mark this execution as supervisor role (for recursion prevention)
export CC_SUPERVISOR_ROLE=supervisor

# Write transient hook bootstrap fallback env file.
...
if [[ -f "$HOOK_ENV_FILE" ]]; then
  rm -f "$HOOK_ENV_FILE"
  log_info "Removed stale hook bootstrap fallback file: $HOOK_ENV_FILE"
else
  log_info "No existing hook bootstrap fallback file to remove"
fi
cat > "$HOOK_ENV_FILE" << EOF
# Transient hook bootstrap fallback variables (auto-generated by supervisor_run.sh)
# Used only when hook callback does not inherit required OPENCLAW_* env values.
# Expected lifecycle: one-time fallback load in on-cc-event.sh, then file deletion.
...
tmux new-session -d -s "$SESSION_NAME" -c "$CLAUDE_WORKDIR" \
  ...
  -e "OPENCLAW_AGENT_ID=${OPENCLAW_AGENT_ID:-}" \
