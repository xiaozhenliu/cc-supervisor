...
人类 ── tmux attach -t cc-supervise[-<id>] ──→ 随时观察 / 手动介入
```
...
# 命名实例示例：tmux attach -t cc-supervise-api
# 退出观察但不关闭会话：Ctrl-B, D
```
...
| `PostToolUse` | 工具成功调用完成 | 默认仅写日志；当前真实回归覆盖 `Bash` 成功路径 |
...
- 安装产物测试与安装失败测试
- 通知队列 fallback 测试
- 真实 `claude + tmux + hook` 的 `PostToolUse` / `Notification` / `Stop` / `SessionEnd` 集成测试
...
当前 `PostToolUse` 集成测试使用 `auto` 模式发送一次成功的 `Bash` 调用（`pwd`），验证 Hook 会真实写入 `logs/events.ndjson`，且默认不会向 OpenClaw 发通知。
...
└── logs/                   # 运行时数据（gitignored）
    ...
    ├── watchdog.pid        # default 实例 watchdog 进程 PID
    ├── instances/<id>/     # 命名实例运行时目录
    └── registry/           # 多实例注册表（projects.json, supervisions.json）
```
...
6. 若是命名实例，改用 `cc-flush-queue --id <id>`，并检查 `logs/instances/<id>/...`
