...
Claude Code 完成一轮响应后，Hook 触发 `on-cc-event.sh`，调用 `openclaw send` 通知。
收到通知后：
...
watchdog 发送 `openclaw send "⏰ watchdog: no activity..."` 告警。
...
**没有收到通知：**
1. 检查 Hook 是否安装：`cat ~/Projects/my-app/.claude/settings.local.json | jq .hooks`
2. 检查 `openclaw` 是否在 PATH：`which openclaw`
3. 检查事件日志：`cat "$CC_SUPERVISOR_HOME/logs/events.ndjson"`
4. 检查脚本日志：`cat "$CC_SUPERVISOR_HOME/logs/supervisor.log" | jq .`
...
**`openclaw` 不在 PATH：**
系统正常降级：所有事件仍写入 `logs/events.ndjson`，不影响日志功能。
