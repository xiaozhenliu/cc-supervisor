...
- **Stop 事件分类处理**：OpenClaw 收到 Stop 通知后，先对 Claude Code 输出内容分类，再按类型响应：
  ...
  | 等待确认 | 输出末尾有 yes/no 或二选一问题 | 告知人类，等待回复后 cc-send | 根据上下文自主回复 |
  | 多项选择 | 输出列出多个选项供选择 | 告知人类，等待回复后 cc-send | 根据任务目标自主选择后 cc-send |
  | 开放性问题 | 输出提出需要具体信息的问题 | 告知人类，等待回复后 cc-send | 若能自主回答则 cc-send，否则升级人类 |
  | 遇到阻塞 | 输出表明错误、权限不足或无法继续 | 告知人类，等待指令 | 尝试自修复一次，失败则升级人类 |
  | 中间状态 | 输出表明任务仍在进行中，无需输入 | 告知人类（仅告知进度） | 发送 "Please continue." |

  **relay 模式**：所有类型均告知人类，等待人类回复后才发送 cc-send，OpenClaw 不自主决策。
  **autonomous 模式**：所有类型由 OpenClaw 自主处理；仅在无法自主处理或任务全部完成时通知人类。
