...
**CRITICAL:** Do NOT poll/sleep/check logs. Wait passively for `[cc-supervisor]` messages. Hooks push notifications automatically. Zero tokens while waiting.

**Exception:** Every 30 minutes, run `cc-flush-queue` to retry queued messages.

#### Stop event classification

Read Claude Code's actual output to see format (y/n, 1/2, a/b, etc.). Use that exact format.

| Type | Decision |
|------|----------|
| Task complete | Notify human → Phase 6 |
| Yes/No | Read format, send "continue" option (y/1/a/yes) |
| Multiple choice | Select recommended/continue option |
| Open question | Answer with defaults or escalate |
| Blocked | Fix once, escalate if repeats |
